...
export default class InMemoryRepositoryRepositorySearcher<TModel> implements RepositorySearcher<TModel> {
    ...
    private async applyFilter(items: TModel[], filter: RepositorySearchDSLFilter<TModel>): Promise<TModel[]> {
        return items.filter((item) => {
            return item[filter.field as string].toLowerCase().includes(filter.value.toLocaleLowerCase());
        });
    }

    private async applySort(items: TModel[], sort: RepositorySearchDSLSorting<TModel>): Promise<TModel[]> {
    ...
    private async applyPaginate(items: TModel[], pagination: RepositorySearchDSLPagination): Promise<TModel[]> {
